...
Output: 1
Explanation: Index 1 is the smallest index such that arr[1] = 2 >= x.

Example 2:
Input: N = 5, arr = {3, 5, 8, 15, 19}, x = 9
Output: 3
Explanation: Index 3 is the smallest index such that arr[3] = 15 >= x.
...
- x larger than all elements (return N)
- Multiple occurrences of x (return the first occurrence's index)
...
Time Complexity: O(N)
- Reason: In the worst case (e.g., x is larger than all elements or no element >= x), 
...
- Since the array is sorted, use binary search to find the smallest index i where 
  arr[i] >= x.
- Maintain a result variable to track the potential lower bound.
...
2. While low <= high:
   ...
   b. If arr[mid] >= x:
      - Update result = mid (potential lower bound).
...
int lowerBoundNaive(vector<int>& arr, int x) {
    int n = arr.size();
...
int lowerBoundOptimal(vector<int>& arr, int x) {
    ...
    while (low <= high) {
        ...
        if (arr[mid] >= x) {
            result = mid; // Update potential lower bound
...
int main() {
    ...
    vector<vector<int>> testCases = {
        ...
        {1, 2, 3},             // x = 4 (x larger than all)
        {1, 2, 3}              // x = 0 (x smaller than all)
    };
    vector<int> xValues = {2, 9, 1, 1, 4, 0};
    vector<int> expected = {1, 3, 0, 0, 3, 0};
...
/*
Expected Output:
Test Case 1 (Naive): x = 2, Result = 1 (Pass)
Test Case 1 (Optimal): x = 2, Result = 1 (Pass)

Test Case 2 (Naive): x = 9, Result = 3 (Pass)
Test Case 2 (Optimal): x = 9, Result = 3 (Pass)

Test Case 3 (Naive): x = 1, Result = 0 (Pass)
Test Case 3 (Optimal): x = 1, Result = 0 (Pass)

Test Case 4 (Naive): x = 1, Result = 0 (Pass)
Test Case 4 (Optimal): x = 1, Result = 0 (Pass)

Test Case 5 (Naive): x = 4, Result = 3 (Pass)
Test Case 5 (Optimal): x = 4, Result = 3 (Pass)
